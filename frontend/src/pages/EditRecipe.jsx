...
const EditRecipe = () => {
    ...
    useEffect(() => {
        const fetchRecipe = async () => {
            try {
                ...
                setTags(data.tags || '');
                setDietaryType(data.dietary_type || 'None');
    ...
    useEffect(() => {
        const fetchImages = async () => {
            if (title.length < 3 || initialLoading) return;
    ...
    if (initialLoading) return <div className="text-center py-20">Loading recipe data...</div>;

    return (
        <div className="max-w-4xl mx-auto px-4 py-10">
            <h1 className="text-4xl font-bold text-slate-800 mb-8">Edit Recipe</h1>
